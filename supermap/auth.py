...
class R:
    def __init__(self, request: Request):
        ...
        query_str = request.query_params.urlencode()
        self.url = "%s://%s%s?%s"%(url_scheme,host,path,query_str)



class AwsV4SignAuth(authentication.BaseAuthentication):
    def authenticate(self, request: Request):
        ACCESS_KEY = settings.AWS_KEY['access_key']
        SECRET_ACCESS_KEY = settings.AWS_KEY['secret_access_key']
        ...
        if signature != req_signature:
            logger.error('Invalidate AwsV4 signature.')
